...
标准见 [v2](TEMPLATE_ACCEPTANCE_STANDARD_V2.md)，具体维护、路由分类、错误登记和人工验收交接顺序见 [模板维护与验收工作流程](TEMPLATE_MAINTENANCE_WORKFLOW.md)；逐项执行使用 [模板通用测试用例](TEMPLATE_TEST_CASES.md)，报告和后端问题分别套用 [测试报告模板](TEMPLATE_TEST_REPORT_TEMPLATE.md)、[Rechard 问题交接模板](TEMPLATE_ISSUE_HANDOFF_TEMPLATE.md)。每次明确分配后用独立TaskSpec及run目录；维护旧模板指定 `page_scope=existing_only`，新模板按确认清单。通用入口沿用 `workflow.py plan-acceptance / browser / assess-acceptance --task <本次任务.json> --run <独立证据目录> --contract <真实页面契约>`。多套逐套运行，不从旧z批次推导新权限。
...
上站模板与开发模板分开。按 Leo 2026-09-21 最新回复，每批从在线 `bing 指数词汇总` 当前分配给 Pony 的**可用模板编号范围**中任选一个，不再默认 `r62`。09-21 `bing体育品牌词` Pony 行 M17 实查为 `z1~z17`，但表格更新后须重新读取，不把截图或当日值写成永久名单；旁边的关键词/系列分组不是模板列表。选版后核对目标模板可用性及真实页面能力，再决定是否重写 TDK。个人 `z18` 开发推送与上站选版无自动关联。现有 TDK 工具仍有 r62 固定门禁，动态分组未接通，未适配前不能说程序已自动执行此新规则。

TDK 草稿新增参考顺序：先核实本批分配词；查看指数词表各个**相关体育页签**，区分竞品文案字段与仅供选词的分组/指数；再只读查看 Leo 历史相近词的上站 TDK 写法；结合本次竞品和目标模板实际内容写新稿，保持团队风格但不复制旧句或无证据承诺，最后做字段查重。所用页签和 Leo 样本日期/行号应随批次留证；指数表和 Leo 行不写入。本补充不改变上站表出现本人域名后才写 E:H 的判据。

域名填入待购买表并回读后，即可核对分配词、模板能力、当次竞品并准备原创 TDK 草稿。上站表出现**对应日期且归属 Pony、域名和分配词唯一匹配的行**，才作为采购完成的流程判据和填写 TDK 的触发条件；待购买表删行/清空不算，仍保留同域名也不单独阻断。仅实际域名/分配词身份矛盾时先核查。确认本人行无现有不同内容后仅写 E:H，回读本批所有字段并保护 A:D/I:P；当前在对话报告，未来机器人接入后由工作平台通知用户完成及行号，用户再手动复制上站。

未来机器人自动上站阶段：逐行读取 M 列 `ServerName` 并核对后台 URL 服务名。用户提供的 `https://s213017.abcd-cms.com/mgradm/optdata.html` 是候选地址，只有该行 M=`s213017` 才匹配；不匹配或不明确则阻断该行，不能沿用旧后台或用其他人的截图赋值。当前只记录规则，机器人尚未接入；不触发脚本复制、后台提交或部署。
