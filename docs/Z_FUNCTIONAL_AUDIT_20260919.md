...
2026-09-20 增量：本报告是**政策2.3的历史快照**。z12集锦链接已按用户提供的可用路由在私有组件修正，z16门户积分榜表头亦已修复；通用政策升级到2.4并新增滚动表头硬门槛。因此下面886状态不能直接作为修改后的2.4验收PASS。z16当前专项证据见 `docs/Z16_PORTAL_STICKY_HEADER_20260920.md`，z12点击证据见 `runs/z12-clips-click-desktop-result.json`。用户明确停止余下404追查。
