...
Leo后续指出的z16球队筛选和相关赛程跑版已专项修复，并按用户纠正建立政策2.2硬门槛：筛选验证实际结果ID，布局验证按具体设计避免挤占后错位/重叠/裁切，不能统一强制Logo对齐。三引擎CLI专项各144状态通过；全页仍有既有加载跳动/样例阻断，未宣称全部验收通过。详情与复跑入口见 `docs/Z16_LEO_REGRESSION_20260919.md`。下方2.1全17套记录为历史，不能升级为2.2通过。
