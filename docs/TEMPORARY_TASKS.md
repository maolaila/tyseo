...
此前 z1–z21 草稿 PR #1657 包含功能、SEO 和数据绑定改动，且包含 z18–z21；应保留为历史草稿，不能直接当作本次 z1–z17 纯样式交付合并。用户要求先启动本地模板逐页人工复查：执行 `./Start-Z-Preview.ps1`，入口 `http://127.0.0.1:8771/`，z1–z17 使用 6301–6317 端口和原业务项目，只在进程环境中选择模板，不修改 `.env`。该入口展示当前修复分支，尚未经过人工验收。
