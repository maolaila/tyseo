...
> 本文下方分支和 PR 编号是当时的历史快照。现行分支已改为 `fix/pony-z1-z17-layout`，草稿 PR 为 [#1664](https://github.com/shuoqiudi/cms-sport-tpl-bing/pull/1664)；旧 PR #1657 已关闭，旧分支已删除。当前复查与未解决项见 `docs/TEMPORARY_TASKS.md`。

> 范围撤回已执行：用户再次明确只保留 z1–z17。提交 `c223e6925` 已撤回 z18–z21 共131个文件的任务改动，并推送原分支。按分支与main共同基线 `6b31ad5c9` 校验，z18–z21 的8个目录树完全一致；z1–z17 的34个目录树与撤回前 `bd7a69d0f` 完全一致。PR现剩346个文件，全部在 `templates/z1..z17`、`static/z1..z17`。下方z1–z21记录均为历史，不代表当前交付范围或z18–z21的现状。
