...
def build(run, contract_run=None):
    ...
    for number in range(1, 22):
        ...
        row = {"template": name, "complete": summary_path.exists(), "records": len(records),
        ...
        if row["complete"] and contracts:
            ...
            row["source_fresh"] = row["source_input_hash"] == fingerprint(
                {"template": name, "commit": commit, "page_types": "all",
    ...
    report = ["# z1–z21 真实应用矩阵", "",
              ...
              "", "自动 `needs_review` 不是 PASS，`blocked` 不从分母删除。图片索引见 `matrix-screenshots.html`，点击可看原图。",
              f"规则失败证据：{len(findings)} 条，详情见 `matrix-overview.json` 的 findings。",
              "", "|模板|采集完毕|源码匹配|记录|状态|", "|---|---|---|---:|---|"]
    for row in rows:
        report.append(f"|{row['template']}|{row['complete']}|{row['source_fresh']}|{row['records']}|{row['status_counts']}|")
...
if __name__ == "__main__":
    ...
    build(args.run, args.contract_run)
