...
def make_plan(task, contract, stamp):
    matrix=task['browser_matrix']; cases=[]; reviews=[]; requirements=[]
    def add(page,engine,width,theme,js,state='observed',scale=1):
    ...
    for page in contract['pages']:
        for w in matrix['widths']:
            for theme in matrix['themes']:add(page,'chromium',w,theme,True)
        for w in matrix['no_js_widths']:add(page,'chromium',w,'light',False)
        if page['page_type_id'] in KEY_PAGES:
            for engine in matrix['secondary_engines']:
                for w in (390,1280):
                    for theme in matrix['themes']:add(page,engine,w,theme,True)
            for w in (320,899,900):
                for theme in matrix['themes']:add(page,'chromium',w,theme,True)
            for w in (390,1280):
                for theme in matrix['themes']:add(page,'chromium',w,theme,True,scale=2)
        ...
        for w in (390,1280):
            for theme in matrix['themes']:
                reviews.append({'page_type_id':page['page_type_id'],'width':w,'theme':theme,'visual':'needs_review','seo':'needs_review'})
    return {'version':2,'input_hash':stamp,'template_id':task.get('target_template_id') or task['reference_template_id'],
...
def assess(plan,records,run,current_hash,reviews=None):
    ...
    for slot in plan['reviews']:
        ...
        if entry.get('input_hash')!=current_hash or not evidence_ok or not entry.get('reviewer') or entry.get('visual')!='pass' or entry.get('seo')!='pass':pending.append(slot)
    ...
    return {'expected':len(expected),'recorded':len(records),'valid':len(valid),'missing':len(missing),'missing_case_ids':missing,
            'plan_stale':plan['input_hash']!=current_hash,
            'invalid':invalid,'unfinished':len(unfinished),'review_pending':len(pending),'contract_pending':contract_pending,
            'ready_for_human_review':not (missing or invalid or unfinished or pending or contract_pending or plan['input_hash']!=current_hash),
...
def write_ledger(plan,out):
    ...
    lines=['# 半自动验收计划','',f"模板 {plan['template_id']}；计划用例 {len(plan['cases'])}；逐页AI复核槽 {len(plan['reviews'])}。",
