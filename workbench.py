    ...
    def scan(self):
        ...
        try:
            with self.lock:
                ...
                m=self.state['monitor'];m.update(status='checking',last_check_at=utc(),next_check_at=None);self.persist()
            ...
            with self.lock:
                ...
                m.update(status='waiting',last_success_at=utc(),last_error=None)
            ...
            with self.lock:
                ...
                m.update(status='blocked',last_error=message);self.persist()
        finally:
            with self.lock:
                m=self.state['monitor'];m['next_check_at']=time.time()+self.interval if m['enabled'] else None;self.persist()
        ...
        if action=='connect':
            def connect():
                try:self.connector.connect()
                    ...
                    with self.lock:self.event('blocked',str(e)[:160]);self.persist()
